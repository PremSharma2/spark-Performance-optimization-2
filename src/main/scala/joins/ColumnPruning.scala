...
object ColumnPruning {
  ...
  val joinExpression: Column = guitarPlayersDF.col("band") === bandsDF.col("id")

  val guitaristsBandsDF: DataFrame = guitarPlayersDF.join(bandsDF, joinExpression, "inner")
  ...
  *(2) BroadcastHashJoin [band#22L], [id#38L], Inner, BuildLeft
  ...
  val guitaristsWithoutBandsDF = guitarPlayersDF.join(bandsDF, joinExpression, "left_anti")
  ...
  val namesDF = guitaristsBandsDF.
    select(guitarPlayersDF.col("name"), bandsDF.col("name"))
  namesDF.explain()
